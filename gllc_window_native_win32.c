...
static LRESULT CALLBACK window_callback(HWND window, UINT msg, WPARAM wparam,
                                        LPARAM lparam) {
  LRESULT result = 0;
  PAINTSTRUCT ps;
  ...
  case WM_LBUTTONDOWN:
    if (w && w->on_mouse_click)
      w->on_mouse_click(w, LOWORD(lparam), HIWORD(lparam), 1, 1,
                        w->on_mouse_click_USER_1);
    return 0;
  case WM_RBUTTONDOWN:
    if (w && w->on_mouse_click)
      w->on_mouse_click(w, LOWORD(lparam), HIWORD(lparam), 2, 1,
                        w->on_mouse_click_USER_1);
    return 0;
  case WM_MBUTTONDOWN:
    if (w && w->on_mouse_click)
      w->on_mouse_click(w, LOWORD(lparam), HIWORD(lparam), 3, 1,
                        w->on_mouse_click_USER_1);
    return 0;
  case WM_LBUTTONUP:
    if (w && w->on_mouse_click)
      w->on_mouse_click(w, LOWORD(lparam), HIWORD(lparam), 1, 0,
                        w->on_mouse_click_USER_1);
    return 0;
  case WM_RBUTTONUP:
    if (w && w->on_mouse_click)
      w->on_mouse_click(w, LOWORD(lparam), HIWORD(lparam), 2, 0,
                        w->on_mouse_click_USER_1);
    return 0;
  case WM_MBUTTONUP:
    if (w && w->on_mouse_click)
      w->on_mouse_click(w, LOWORD(lparam), HIWORD(lparam), 3, 0,
                        w->on_mouse_click_USER_1);
    return 0;
  case WM_SIZE:
    if (w && w->on_size)
  ...
  case WM_MOUSEMOVE:
    if (w && w->on_mouse_move)
      w->on_mouse_move(w, LOWORD(lparam), HIWORD(lparam),
                       w->on_mouse_move_USER_1);

    return 0;
  case WM_MOUSEWHEEL:
    if (w && w->on_mouse_scroll)
      w->on_mouse_scroll(w, 0, GET_WHEEL_DELTA_WPARAM(wparam) / 120,
                         w->on_mouse_scroll_USER_1);
    return 0;

  case WM_MOUSEHWHEEL:
    if (w && w->on_mouse_scroll)
      w->on_mouse_scroll(w, GET_WHEEL_DELTA_WPARAM(wparam) / 120, 0,
                         w->on_mouse_scroll_USER_1);
    return 0;
  default:
...
struct gllc_WN *gllc_WN_create(void *parent) {
  ...
  if (wn) {
    ...
    wn->w = CreateWindowEx(0, MAKEINTATOM(wndClass), "OpenGL Window",
                           WS_CHILD | WS_VISIBLE, rc.left, rc.top,
                           rc.right - rc.left, rc.bottom - rc.top, (HWND)parent,
...
void gllc_WN_on_paint(struct gllc_WN *w, gllc_WN_paint_cb on_paint,
                      void *USER_1) {
  w->on_paint = on_paint;
...
void gllc_WN_on_mouse_move(struct gllc_WN *w,
                           gllc_WN_mouse_move_cb on_mouse_move, void *USER_1) {
  w->on_mouse_move = on_mouse_move;
  w->on_mouse_move_USER_1 = USER_1;
}

void gllc_WN_on_mouse_click(struct gllc_WN *w,
...
void gllc_WN_on_mouse_scroll(struct gllc_WN *w,
...
void gllc_WN_swap_buffers(struct gllc_WN *w) { SwapBuffers(w->dc); }

void gllc_WN_dirty(struct gllc_WN *w) { InvalidateRect(w->w, NULL, TRUE); }