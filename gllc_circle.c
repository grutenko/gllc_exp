...
void gllc_circle_build(double x, double y, double radius, GLfloat *color,
                       GLfloat *fcolor, struct gllc_DE *DE_bound,
                       struct gllc_DE *DE_fill, int filled) {
  GLfloat V[2 * N];
  GLuint I[N];
...
#define THETA(index) (2.0 * PI * (index) / N)
  ...
  DE_conf.V = V, DE_conf.I = I, DE_conf.color = color, DE_conf.V_count = N,
  DE_conf.I_count = N;
...
static void _build(struct gllc_block_entity *ent, double scale,
  ...
  gllc_circle_build(circle->x, circle->y, circle->radius, color_, fcolor_,
                    circle->DE_bound, circle->DE_fill,
...
static int _picked(struct gllc_block_entity *ent, double scale, double x,
  ...
  double dy = fabs(y - c->y);

  if (GLLC_ENT_FLAG(ent, GLLC_ENT_FILLED)) {
    return sqrt(dx * dx + dy * dy) <= c->radius;
  } else {
    return sqrt(dx * dx + dy * dy) <= c->radius + (2.0f * scale) &&
           sqrt(dx * dx + dy * dy) >= c->radius - (2.0f * scale);
  }
}
...
struct gllc_circle *gllc_circle_create(struct gllc_block *block, double x,
