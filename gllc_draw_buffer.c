...
void gllc_DE_dump(const struct gllc_DE *de) {
  ...
  printf("  color: (%f, %f, %f, %f)\n", de->color[0], de->color[1],
         de->color[2], de->color[3]);

  printf("  BBox: [%f, %f] - [%f, %f]\n", de->BBox_x0, de->BBox_y0, de->BBox_x1,
         de->BBox_y1);
...
void gllc_DBG_build(struct gllc_DBG *DBG, struct gllc_DBD *DBD) {
  ...
  if (DBG->DE_cap <= DE_size) {
    struct gllc_DBG_DE *DE_new =
        realloc(DBG->DE, sizeof(struct gllc_DBG_DE) * DE_size);
    if (!DE_new)
      return;
  ...
  int i = 0, j;
  for (DE = DBD->DE_head; DE; DE = DE->next) {
    if (DE->skip) {
      DE = DE->next;
      continue;
    }

    memcpy(DBG->V + VBO_offset, DE->V_cache,
           sizeof(GLfloat) * 2 * DE->V_cache_count);

    for (j = 0; j < DE->I_cache_count; j++) {
      DE->I_cache[j] += V_nth;
    }

    memcpy(DBG->I + EBO_offset, DE->I_cache,
           sizeof(GLuint) * DE->I_cache_count);
