...
void build_vertices(struct gllc_block_entity *ent, const double *ver,
                    size_t ver_count) {
  int i;

  if (ent->DE_vertices_cap < ver_count) {
    int new_DE_cap = ver_count;
    struct gllc_DE **new_DE =
        realloc(ent->DE_vertices, sizeof(struct gllc_DE *) * new_DE_cap);
    if (!new_DE)
      return;
  ...
  for (i = 0; i < ver_count; i++) {
    ...
    GLfloat V[] = {
        (GLfloat)ver[i * 2] - 3.0f, (GLfloat)ver[i * 2 + 1] - 3.0f,
        (GLfloat)ver[i * 2] + 3.0f, (GLfloat)ver[i * 2 + 1] - 3.0f,
        (GLfloat)ver[i * 2] + 3.0f, (GLfloat)ver[i * 2 + 1] + 3.0f,
        (GLfloat)ver[i * 2] - 3.0f, (GLfloat)ver[i * 2 + 1] + 3.0f,
    };
...
void gllc_ent_build(struct gllc_block_entity *ent, double scale,
                    struct gllc_DBD *DBD) {
  ent->vtable->build(ent, scale, DBD);
}

int gllc_ent_bbox(struct gllc_block_entity *ent, double scale, double *x0,
                  double *y0, double *x1, double *y1) {
  return ent->vtable->bbox ? ent->vtable->bbox(ent, scale, x0, y0, x1, y1) : 0;
}

int gllc_ent_picked(struct gllc_block_entity *ent, double scale, double x,
                    double y) {
  return ent->vtable->picked ? ent->vtable->picked(ent, scale, x, y) : 0;
}