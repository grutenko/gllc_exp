...
void gllc_W_grid_configure(struct gllc_W_grid *grid,
                           struct gllc_W_grid_config *config) {
  if (config->offset) {
...
void gllc_W_grid_draw(struct gllc_W_grid *grid, GLuint u_color_loc) {
  ...
  float color[4] = {
      ...
      grid->color[2] + (grid->clear_color[2] - grid->color[2]) * 0.5f, 1.0f};

  glUniform4f(u_color_loc, color[0], color[1], color[2], color[3]);
  glDrawArrays(GL_LINES, 0, grid->V0_count);

  glUniform4f(u_color_loc, grid->color[0], grid->color[1], grid->color[2],
              grid->color[3]);
