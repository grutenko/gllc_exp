...
static void build_without_geometry(struct gllc_DE *DE_bound,
                                   struct gllc_DE *DE_fill, float *color,
                                   float *fcolor, int closed, int filled) {
...
void gllc_polyline_build(const struct gllc_polyline_vertex *V, size_t V_count,
                         struct gllc_DE *DE_bound, struct gllc_DE *DE_fill,
                         float *color, float *fcolor, int closed, int filled) {
  ...
  if (closed && filled) {
    ...
    if (tess) {
      ...
      if (tessTesselate(tess, TESS_WINDING_POSITIVE, TESS_POLYGONS, 3, 2,
                        NULL)) {
...
static void _build(struct gllc_block_entity *ent, double scale,
  ...
  if (GLLC_ENT_FLAG(ent, GLLC_ENT_GEOMETRY_MODIFIED)) {
    gllc_polyline_build(pline->ver, pline->ver_size, pline->DE_bound,
                        pline->DE_fill, color_, fcolor_,
                        GLLC_ENT_FLAG(ent, GLLC_ENT_CLOSED),
                        GLLC_ENT_FLAG(ent, GLLC_ENT_FILLED));
  } else {
    build_without_geometry(pline->DE_bound, pline->DE_fill, color_, fcolor_,
                           GLLC_ENT_FLAG(ent, GLLC_ENT_CLOSED),
                           GLLC_ENT_FLAG(ent, GLLC_ENT_FILLED));
  }
}
...
static int _selected(struct gllc_block_entity *ent, double scale, double x0,
  ...
  int ok = _bbox(ent, scale, &bbox_x0, &bbox_y0, &bbox_x1, &bbox_y1);
...
static int push_ver(struct gllc_polyline *pline, double x, double y) {
  if (pline->ver_size + 2 > pline->ver_cap) {
    ...
    struct gllc_polyline_vertex *new_ver =
        (struct gllc_polyline_vertex *)realloc(
            pline->ver, sizeof(struct gllc_polyline_vertex) * new_cap);
    if (!new_ver) {
      return 0;
    }
    pline->ver = new_ver;
