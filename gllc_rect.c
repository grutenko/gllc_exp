...
void gllc_rect_build(double x, double y, double width, double height,
                     double angle, GLfloat *color, GLfloat *fcolor,
  ...
  GLfloat V[] = {x0, y0, x1, y0, x1, y1, x0, y1};
...
static void _build(struct gllc_block_entity *ent, double scale,
  ...
  if (GLLC_ENT_FLAG(ent, GLLC_ENT_GEOMETRY_MODIFIED)) {
    gllc_rect_build(rect->x, rect->y, rect->width, rect->height, rect->angle,
                    color_, fcolor_, rect->DE_bound, rect->DE_fill,
                    GLLC_ENT_FLAG(ent, GLLC_ENT_FILLED));
  } else {
    build_without_geometry(rect->DE_bound, rect->DE_fill, color_, fcolor_,
                           GLLC_ENT_FLAG(ent, GLLC_ENT_FILLED));
  }
}
...
static int _picked(struct gllc_block_entity *ent, double scale, double x,
  ...
  return r->x <= x && r->y <= y && (r->x + r->width) >= x &&
         (r->y + r->height) >= y;
}
...
struct gllc_rect *gllc_rect_create(struct gllc_block *block, double x, double y,
