...
#define TIMER_INTERVAL 64
  ...
  struct gllc_block *hBlock =
      gllc_drw_add_block(hDrw, "Model Space", 0.0f, 0.0f);
  ...
  /*for (int y = 0; y < height; y += grid_size)
  {
          for (int x = 0; x < width; x += grid_size)
          {
                  ...
                  struct gllc_rect *hRect = gllc_block_add_rect(hBlock, x0, y0,
  (double)grid_size, (double)grid_size, 0.0f, 1);
                  ...
                  //struct gllc_point *hPoint = gllc_block_add_point(hBlock, x0
  + (double)grid_size / 2, y0 +  + (double)grid_size / 2);
          }
  }*/
  ...
  while (!feof(plines)) {
    ...
    for (i = 0; i < n_points; i++) {
      ...
      if (sscanf(line, "%lf %lf %lf %lf %lf", &x, &y, &z, &errRad, &energy) !=
          5) {
        goto __end;
      }
      ...
      // struct gllc_circle *circle = gllc_block_add_circle(hBlock, x, y,
      // log2(energy), 1); gllc_ent_set_color((struct gllc_block_entity
      // *)circle, color); gllc_ent_set_fcolor((struct gllc_block_entity
      // *)circle, color);
      struct gllc_point *point = gllc_block_add_point(hBlock, x, y);
      gllc_ent_set_color((struct gllc_block_entity *)point, color);
    }
  }
