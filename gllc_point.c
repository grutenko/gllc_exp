...
static void _build(struct gllc_block_entity *ent, double scale,
  ...
  GLfloat V[] = {(GLfloat)point->x - 3.0f, (GLfloat)point->y,
  ...
  struct gllc_DE_config DE_conf = {.V = V,
                                   .I = I,
                                   .V_count = 4,
                                   .I_count = 4,
                                   .color = color_,
...
static int _bbox(struct gllc_block_entity *ent, double scale, double *bbox_x0,
  ...
  *bbox_x0 = point->x - 3.0f * scale;
  *bbox_y0 = point->y - 3.0f * scale;
  *bbox_x1 = point->x + 3.0f * scale;
  *bbox_y1 = point->y + 3.0f * scale;
...
static int _picked(struct gllc_block_entity *ent, double scale, double x,
  ...
  double x0 = point->x - 3.0f * scale;
  double y0 = point->y - 3.0f * scale;
  double x1 = point->x + 3.0f * scale;
  double y1 = point->y + 3.0f * scale;
...
static int _vertices(struct gllc_block_entity *ent, double scale, double *ver) {
  int cnt = 1;
  if (ver) {
    ver[0] = ((struct gllc_point *)ent)->x;
    ver[1] = ((struct gllc_point *)ent)->y;
  }
  return cnt;
}
...
struct gllc_point *gllc_point_create(struct gllc_block *block, double x,
                                     double y) {
