...
static inline void render_camera(struct gllc_window *w) {
  ...
  glm_ortho(-half_w, half_w, -half_h, half_h, -1000.0f, 1000.0f,
            w->GL.GL_m_proj);

  glm_ortho(0.0f, (float)w->viewport.width, (float)w->viewport.height, 0.0f,
            -1.0f, 1.0f, w->GL.GL_m_proj_screen);
...
static void update_viewport(struct gllc_window *w) {
  ...
  gllc_window_wnd_to_drw(w, (double)w->viewport.width,
                         (double)w->viewport.height, &v.x1, &v.y1);
  v.scale = w->viewport.scale_factor;

  struct gllc_W_grid_config conf = {.clear_color = NULL,
                                    .color = NULL,
                                    .gap = NULL,
                                    .offset = NULL,
                                    .viewport = &v};
...
static void on_mouse_click(struct gllc_WN *wn, int x, int y, int mode,
                           int action, void *USER_1) {
  ...
  if (mode == 3) {
    ...
    if (action == 1) {
      w->UI.sel_x0 = (double)(x - (int)(w->viewport.width / 2)) *
                         w->viewport.scale_factor -
                     w->viewport.dx;
      w->UI.sel_y0 =
          (double)((w->viewport.height - y) - (int)(w->viewport.height / 2)) *
              w->viewport.scale_factor -
          w->viewport.dy;
      ...
      if (!(w->flags & GLLC_WINDOW_SELECTION)) {
        ...
        gllc_block_ent_select_by_bbox(w->block, w->UI.sel_x0, w->UI.sel_y0,
                                      w->UI.sel_x1, w->UI.sel_y1);
      }
...
static void on_mouse_move(struct gllc_WN *wn, int x, int y, void *USER_1) {
  ...
  if (w->flags & GLLC_WINDOW_SELECTION) {
    w->UI.sel_x1 =
        (x - (int)(w->viewport.width / 2)) * w->viewport.scale_factor -
        w->viewport.dx;
    w->UI.sel_y1 = ((w->viewport.height - y) - (int)(w->viewport.height / 2)) *
                       w->viewport.scale_factor -
                   w->viewport.dy;
  }
...
static void on_mouse_scroll(struct gllc_WN *wn, int dx, int dy, void *USER_1) {
  ...
  if ((w->viewport.scale_factor > 300.0f && dy > 0) ||
      (w->viewport.scale_factor < 0.005f && dy < 0))
    return;
  ...
  w->viewport.dx += (mouse_x - (int)(w->viewport.width / 2)) *
                    (w->viewport.scale_factor - old_scale);
  w->viewport.dy +=
      ((w->viewport.height - mouse_y) - (int)(w->viewport.height / 2)) *
      (w->viewport.scale_factor - old_scale);
  if (w->block) {
...
static size_t draw_DBG(GLuint color_loc, GLuint flags_loc,
                       GLuint center_point_loc, struct gllc_DBG *DBG,
                       GLfloat wx0, GLfloat wy0, GLfloat wx1, GLfloat wy1) {
  int i;
  for (i = 0; i < DBG->DE_size; i++) {
    if (!(DBG->DE[i].BBox_x1 > wx0 && DBG->DE[i].BBox_y1 > wy0 &&
          DBG->DE[i].BBox_x0 < wx1 && DBG->DE[i].BBox_y0 < wy1)) {
      continue;
    }

    glUniform1ui(flags_loc, DBG->DE[i].flags);
    glUniform2f(center_point_loc, DBG->DE[i].center_point[0],
                DBG->DE[i].center_point[1]);
    glUniform4f(color_loc, DBG->DE[i].color[0], DBG->DE[i].color[1],
                DBG->DE[i].color[2], DBG->DE[i].color[3]);

    glDrawElements(DBG->DE[i].GL_type, DBG->DE[i].size, GL_UNSIGNED_INT,
...
static void draw(struct gllc_window *w) {
  ...
  glUniformMatrix4fv(w->GL.GL_u_MVP_loc, 1, GL_FALSE,
                     (const float *)w->GL.GL_m_MVP);

  GLfloat viewport[2] = {(GLfloat)w->viewport.width,
                         (GLfloat)w->viewport.height};
  ...
  gllc_window_wnd_to_drw(w, (double)w->viewport.width,
                         (double)w->viewport.height, &wx1, &wy1);

  draw_DBG(w->GL.GL_u_color_loc, w->GL.GL_u_flags_loc,
           w->GL.GL_u_center_point_loc, &w->DBG.DBG, wx0, wy0, wx1, wy1);

  glBindVertexArray(w->DBG.DBG_interactive.VAO);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, w->DBG.DBG_interactive.EBO);
  draw_DBG(w->GL.GL_u_color_loc, w->GL.GL_u_flags_loc,
           w->GL.GL_u_center_point_loc, &w->DBG.DBG_interactive, wx0, wy0, wx1,
           wy1);

  if (w->flags & GLLC_WINDOW_SELECTION) {
    gllc_W_selection_draw(&w->UI.selection, w->GL.GL_u_color_loc, w->UI.sel_x0,
                          w->UI.sel_y0, w->UI.sel_x1, w->UI.sel_y1);
  }

  glUniformMatrix4fv(w->GL.GL_u_MVP_loc, 1, GL_FALSE,
                     (const float *)w->GL.GL_m_MVP_screen);

  glBindVertexArray(w->DBG.DBG_screen.VAO);
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, w->DBG.DBG_screen.EBO);
  draw_DBG(w->GL.GL_u_color_loc, w->GL.GL_u_flags_loc,
           w->GL.GL_u_center_point_loc, &w->DBG.DBG_screen, wx0, wy0, wx1, wy1);

  gllc_W_cursor_draw(&w->UI.cursor, w->GL.GL_u_color_loc, w->UI.cursor_x,
                     w->UI.cursor_y, w->viewport.width, w->viewport.height);
...
void gllc_window_set_clear_color(struct gllc_window *window, int r, int g,
                                 int b) {
  ...
  struct gllc_W_grid_config conf = {.clear_color = window->clear_color};

  gllc_W_grid_configure(&window->UI.grid, &conf);
}

void gllc_window_set_size(struct gllc_window *window, int x, int y, int width,
                          int height) {
...
void gllc_window_grid_configure(struct gllc_window *window, double gap_x,
                                double gap_y, double offset_x, double offset_y,
                                float *color) {
...
void gllc_window_wnd_to_drw(struct gllc_window *w, double x, double y,
                            double *xd, double *yd) {
  *xd = (x - ((double)w->viewport.width / 2)) * w->viewport.scale_factor -
        w->viewport.dx;
  *yd = (y - ((double)w->viewport.height / 2)) * w->viewport.scale_factor -
        w->viewport.dy;
}
...
void gllc_window_zoom_bb(struct gllc_window *window) {
  ...
  if (fabs(x_max - x_min) > fabs(y_max - y_min)) {
    window->viewport.scale_factor =
        fabs(x_max - x_min) / (double)window->viewport.width;
  } else {
    window->viewport.scale_factor =
        fabs(y_max - y_min) / (double)window->viewport.height;
  }
